...
file = r'C:\Users\10283.CENACE\Dropbox\Tesis_Rodrigo_Reyes\Projects\Evento SIN 28 dic 2020\MultiERA\freq.csv'
...
yMean = np.zeros([st.shape[0] , st.shape[1]])
y = np.zeros([st.shape[0] , st.shape[1]])
for j in range(st.shape[1]):
	yMean[: , j] = st[: , j]
	yMean[: , j] = yMean[: , j] - np.mean(yMean[: , j])
	st[:,j] = yMean[: , j]
...
for j in range(r):
	for i in range(r):
		H[can*i: can*(i+1) , j] = st[j+i+1 , :]

U, s, v = np.linalg.svd(H)
S = np.diag(s)
V = v.T

modes = Energy_Threshold(s, 35.0)
if modes !=4:
	print('Modos diferentes a 4, modes:' + str(modes))
	sys.exit()
...
print('ievdhm', t1_ievdhm - t0_ievdhm)
print('tkeo', t1_tkeo - t0_tkeo)
